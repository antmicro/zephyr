...
Xilinx KV260 Development Board RPU Cortex-R8
############################################
...
* Two independent R8 cores with their own TCMs (tightly coupled memories)
* Or as a single dual lock step unit with double the TCM size.

This processing unit is based on an ARM Cortex-R8 CPU, it also enables the following devices:
...
* Only the first core of the R8 subsystem is supported.
* Xilinx Zynq TTC driver does not support tickless mode operation.
* The Cortex-R8 and the Cortex-A53 shares the same UART controller, more details below.
...
.. code-block:: console
        ...
        *** Booting Zephyr OS build v3.4.0  ***
        Hello World kv260_r8!

References
**********

1. ARMv7-A and ARMv7-R Architecture Reference Manual (ARM DDI 0406C ID051414)
2. Cortex-R8 and Cortex-R8F Technical Reference Manual (ARM DDI 0460C ID021511)
